...
class XbRichTextComponent extends Taro.PureComponent {
  ...
  render0(block, index) {
    let dom_block = null;
    const children = block.children && block.children.map((child, i) => {
    ...
    if (block.type === 'text') {
      ...
      dom_block = <View className={block.className} key={index} onClick={this.onLinkClick.bind(this, block)}>
          {children}
        </View>;
    }
    return dom_block;
  ...
  render() {
    const { richText, raw, customStyle, type } = this.props;
